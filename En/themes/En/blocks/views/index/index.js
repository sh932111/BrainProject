...
ctrl.startup = function() {
	ctrl.wordManagerBk = new ctrl.wordManagerBlock();
	var query = ctrl.bi.query;
	var enTypeID = query.enTypeID;
	if (enTypeID) {
		ctrl.wordManagerBk.initWord(enTypeID);
		ctrl.wordManagerBk.addWord();
	}
	ctrl.sel('#dropdown-btn-type').dropdown({
		inDuration: 300,
		outDuration: 225,
	...
	ctrl.sel('#modal-trigger').leanModal();
};

ctrl.wordManagerBlock = function() {
	...
	this.addWord = function() {
		var word_manager = new ctrl.wordManager(this.wordManagerList.length);
		this.wordManagerList.push(word_manager);
		var group = ctrl.sel("#word-group");
	...
	this.editWord = function(enWordID) {
		goPage("/word/edit?enWordID=" + enWordID);
	};
	this.chooseType = function(enTypeID) {
		goPage("/?enTypeID=" + enTypeID);
	}; 
};

ctrl.wordManager = function(root_id) {
	this.rootID = root_id;
	...
	this.getAddWord = function() {
		var hrf =  "javascript: _wf.getCtrl('" + ctrl.getBlockID() + "').";
		var div  = document.createElement('div');
		div.className = "input-field col s1";
		var a  = document.createElement('a');
		a.className = "btn-floating btn-large waves-effect waves-light blue";
		a.setAttribute('href', hrf + 'wordManagerBk.addConfig(' + this.rootID + ');');
