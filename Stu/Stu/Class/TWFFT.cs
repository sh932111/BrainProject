...
namespace Stu.Class
{
    public class TWFFT
    {
        ...
        public static Dictionary<string, float[]> FFT(float[] xreal, float[] ximag)
        {
            //n值为2的N次方
            int n = 2;
            ...
            result.Add("xreal", xreal);
            result.Add("ximag", ximag);
            return result;
        }

        /*FFT 複數取絕對值*/
        public static ArrayList FFTAbs(Dictionary<string, float[]> resource)
        {
            float[] xreal = resource["xreal"];
            float[] ximag = resource["ximag"];
            int max = 2048;
            ...
            for (int i = 0; i < max; i++)
            {
                float a = xreal[i];
                float b = ximag[i];
                double _res = Math.Sqrt(a * a + b * b);
                res.Add(_res + "");
            }
            return res;
        }

        public static ArrayList FFTMagic(ArrayList one, ArrayList two, int row_count)
        {
